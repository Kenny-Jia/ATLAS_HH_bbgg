...
void llgg(const char * inputFile) {
    ...
    for (Long64_t entry = 0; entry < nEntries; ++entry) {
        ...
        */
        ...
        if (TMath::Abs(z_ll.DeltaR(hjet -> P4()) - TMath::Pi()) > 0.2) {
            //continue;
        }
    ...
    TCanvas * hetacanvas = new TCanvas("hetacanvas", "Canvas", 1400, 1400, 1400, 1400);
    hetacanvas -> SetWindowSize(1204, 1228);
    hetacanvas -> SetCanvasSize(1200, 1200);
    Hetahisto -> Draw("HIST");
    hetacanvas -> SaveAs((channelName + "_anti_heta.png").c_str());
